...
class LoginUser : AppCompatActivity() {
    ...
    private lateinit var weixinNumber: EditText
    private lateinit var password: EditText
    private lateinit var phone_login: TextView
    private lateinit var button: Button
    private val myHandler: MyHandler = MyHandler()

    companion object { // 相当于是Java中的private static final
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initViews()
        val number: String? = intent.getStringExtra("weixin_number")
        weixinNumber.setText(number)
        button.isEnabled = !(weixinNumber.text.toString() == "" || password.text.toString() == "")
        inputFocus()
        buttonChangeColor()
        phone_login.setOnClickListener {
            val intent: Intent = Intent(this@LoginUser, LoginPhone::class.java)
            startActivity(intent)
        }
        button.setOnClickListener {
            val intent = Intent()
            intent.setClass(this@LoginUser, Loading::class.java)
            startActivity(intent)
            Thread {
                try {
                    Thread.sleep(1000)
                    httpUrlConnPost(weixinNumber.text.toString(), password.text.toString())
    ...
    private fun httpUrlConnPost(number: String, password: String): Unit {
    ...
    fun login_activity_back(v: View) {
        val intent: Intent = Intent()
        intent.setClass(this@LoginUser, Welcome::class.java)
        startActivity(intent)
        finish()
    }
        ...
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                1 -> {
                    ...
                    Toast.makeText(applicationContext, "登录成功", LENGTH_SHORT).show()
                    val intent: Intent = Intent()
                    intent.putExtra("weixin_number", weixinNumber.text.toString())
                    intent.setClass(this@LoginUser, MainWeixin::class.java)
                    startActivity(intent)
                    finish()
                }
                2 -> {
                    Log.i(TAG, msg.what.toString())
                    AlertDialog.Builder(this@LoginUser)
                        .setTitle("                  登录失败")
